...
public class UserFriendlySummary implements TestWatcher, BeforeAllCallback, AfterAllCallback {
    ...
    public void afterAll(ExtensionContext context) {
        ...
        if (!abortedTests.isEmpty()) {
            System.out.println("\n⚠️  Aborted Tests:");
            abortedTests.forEach(test -> System.out.println("  - " + test));
        }
        
        if (!skippedTests.isEmpty()) {
            System.out.println("\n⏭️  Skipped Tests:");
            skippedTests.forEach(test -> System.out.println("  - " + test));
        }
